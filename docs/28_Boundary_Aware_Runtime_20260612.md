...
This document belongs to GyroOS.
...
GyroOS uses these concepts as runtime classifications.
It must not modify the Gyro Logic definitions.
...
These are runtime classifications.
They are not authorization decisions, authentication decisions, or direct error states.
...
These are response candidates.
BoundaryState does not automatically determine Response.

---

## 8. Response Selection

BoundaryState is a response-orientation factor.
It narrows the response space but does not decide the response.
